...
%%%%% FIN RENGLÓN 2
  ...
  Distancia \emph{City block}     &  
   ...
   \includegraphics[width=50mm]{city-block.png}
   &  Numéricos
...
\begin{center}
\includegraphics[width=10.5cm]{single_linkage.png}
\end{center}
...
\begin{center}
\includegraphics[width=10.5cm]{complete-linkage.png}
\end{center}
...
\begin{center}
\includegraphics[width=10.5cm]{average_linkage.png}
\end{center}
...
\begin{center}
\includegraphics[width=10.5cm]{wards_linkage.png}
\end{center}
...
```{r visualizar_matriz, collapse=TRUE, echo=TRUE, fig.pos = 'H', fig.align = "center", message=FALSE, fig.cap = 'Heatmap de la matriz de distancias' }
library(factoextra)
...
```{r dendograma_average, include = TRUE, echo = TRUE, fig.pos = 'H', fig.align = "center", out.width="80%", warning=FALSE, message=FALSE, fig.cap = 'con el método del promedio.'}
...
```{r dendograma_Ward, include = TRUE, echo = TRUE, fig.pos = 'H', fig.align = "center",out.width="80%", warning=FALSE, message=FALSE, fig.cap = 'con el método Ward.' }
...
```{r dendograma_single, include = TRUE, echo = TRUE, fig.pos = 'H', fig.align = "center",out.width="80%", warning=FALSE, message=FALSE, fig.cap = 'con el método de la liga más sencilla.' }
...
```{r dendograma_complete, include = TRUE, echo = TRUE, fig.pos = 'H', fig.align = "center",out.width="80%", warning=FALSE, message=FALSE, fig.cap = 'con el método de la liga completa.' }
...
\begin{center}
\includegraphics[width=15.5cm]{PCA_3.png}
\end{center}
...
Supongamos que tenemos datos en un espacio de $p$ dimensiones, entonces lo que se busca es una transformación lineal $a_1'X=(a_{11},\ a_{12},...,\ a_{1p})(X_1,\ X_2,...,\ X_p)=Y_1$ de manera que tenga la máxima varianza, a la que se le conoce como componente principal y está dada por:

\[ Y_1\ = \ a_{11}X_1 +\ a_{12}X_2\ +\ ...\ + a_{1p}X_p . \]

Lo que se busca, es elegir $a_1=(a_{11},\ a_{12},...,\ a_{1p})$ de manera que la $var(Y_1)$  tenga la máxima varianza. Se sabe que si el vector $X=(X_1,\ X_2,...,\ X_p)$ tiene matriz de varianzas y covarianzas $\Sigma$, entonces: 
\[ var(Y)=var(a'X)=var(a'\Sigma\ a).\]

Si se elige un $a_1$ que tenga una norma muy grande, entonces la varianza de $Y_1$ podría resultad muy grande también. Entonces es necesario imponer condiciones sobre $a_1$ para acotar el tamaño de la varianza de $Y_1$ y no hacerla muy grande de manera arbitraria. La condición que se impone es:
\[||a_1||=a_1'\cdot a_1=1.\]

Para lograr maximizar con esta condición, se utilizan los multiplicadores de Lagrange:

  - Maximizar $f(x_1,x_2,...,x_p)$ sujeto a $g(x_1,x_2,...,x_p)=c$, donde $f$ es una función diferenciable.
...
```
Este procedimiento elige las variables SmokingStatus y Sex. La siguiente selección selección utiliza la función `ordiR2step()`. La configuración por defecto de este procedimiento para incluir una nueva variable se basa en R adj y su comparación con la del modelo global (con todas las variables). La selección se detendrá si la nueva variable no es significativa o el R<sup>2</sup>adj del modelo que incluye esta nueva variable superaría el R2 adj del modelo global.
```{r}
...
La primera sección después de la "Call:" da los "coeficientes de contingencia media al cuadrado" del análisis. En el CCA desempeñan el mismo papel que la varianza total en la RDA. Como la matriz de comunidad se convierte en distancia $\chi^{2}$, las entradas son coeficientes de contingencia. La variación total antes de someter la matriz a regresión ponderada es de 4,9330; ésta es la variación que podría explicarse. La variación en la matriz de la comunidad que se explica después de la regresión ponderada es 0,3782; esta es la variación que será explicada por los ejes en el CCA. La varianza de los residuos de la regresión es de 4,5548; se trata de la variación que no será explicada por los ejes en el CCA, que puede ser sometida a CA. 
Aquí, observamos que el CCA no tiene mucho éxito porque sólo 0,3782/4,9330 o 0,0767 (véase la columna Proporción y la fila Restringida) de la variación total de datos fue capturada en el CCA por las cuatro variables. La varianza explicada por ejes particulares puede encontrarse mediante la función `summary()`:
```{r}
summary(smoker_cca)
```
...
Al igual que en RDA, realicemos una prueba de permutación para el modelo global, cada eje y cada variable explicativa. Para asegurarse de obtener el mismo resultado de la prueba de permutación cada vez que se ejecute, establezca la misma semilla.
```{r}
set.seed (123)
anova(smoker_cca, step=1000)  # El resultado no es estadísticamente significativo
# Entonces una prueba de significancia por cada eje
anova (smoker_cca, by = 'axis',step=1000) # El primer eje es significante
# Ahora probamos la signifiancia de cada variable explicativa
anova (smoker_cca, by = 'terms')
# Vemos que SmokingStatus es significativo con un p-value de 0.009
```
...
```
La condición de fumador alcanza el mismo nivel de significación con un valor p de 0,007. Sin embargo, el modelo de parsimonia ha dado sus frutos con un residuo mayor.
...
El CAP básico se puede realizar con los siguientes códigos. Las variables de restricción incluyen dos variables binarias (SmokingStatus y Sex), y dos variables continuas (PackYears y Edad). La condición de término especial (Edad) se utiliza para particionar el efecto de la edad.
```{r}
throat_meta <- select(throat.meta, SmokingStatus, Age, Sex, PackYears)
...
anova(throat_cap)
```
El modelo es estadísticamente significativo con un valor p de 0.004.
La función `plot()`, genera la siguiente figura, pero no es informativa
```{r}
plot(throat_cap)
```
...
# un valor p de 0.001 y margialmente significativo en el segundo eje con un 
# valor p de 0.073.
# Ahora probamos con cada valiable explicativa
anova(throat_cap, by="terms", step=1000)
```
Podemos ver que tanto SmokingStatus como Sex son significativos con valores p = 0,003 y 0,035, respectivamente.
Ahora, hagamos la selección hacia delante utilizando la función `ordistep()` del paquete `vegan`. Como dijimos en RDA y CCA, la función compara las variables basándose en el criterio AIC y los valores p de la prueba de permutación de Monte Carlo.
```{r}
